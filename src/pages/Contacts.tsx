...
function ImportModal({ onDone, onClose }: { onDone:()=>void; onClose:()=>void }) {
  ...
  async function doImport() {
    ...
    try { const r=await apiFetch<any>('/api/contacts/import',{json:{csv}}); setResult(r) }
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
        ...
        <div className="p-5 space-y-4">
          {!result ? (
            <>
              <p className="text-xs text-sage-500">CSV columns: <code className="bg-sage-50 px-1 rounded">email, first_name, last_name, company, phone, location, tags</code></p>
              ...
              <div className="flex gap-3">
                ...
                <button onClick={doImport} disabled={loading||!csv.trim()} className="btn-primary flex-1">
...
export default function Contacts() {
  ...
  function exportCsv() { window.open('/api/contacts/export','_blank') }
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        ...
        <div className="flex gap-2">
          <button className="btn-ghost" onClick={exportCsv}><Download className="w-4 h-4"/>Export</button>
